...
/*	Design a module that takes in a single arg, char *path,
	...
	write it to sbuf. Uses S_IFDIR() on sbuf.st_mode to see 
...
/*	Design a module that takes in a single arg, char *path,
	...
	Uses the Stat struct to construct a struct, sbuf,
	and uses stat() to obtain information from the file and
	write it to sbuf. Uses S_IFREG on sbuf.st_mode to see if 
	the file is regular. A 1 is returned if the S_IFREG is true
